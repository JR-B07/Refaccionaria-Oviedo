# Visual Guide - Nuevo Cierre de Caja

## 📱 Vista 1: Lista de Cierres (cajas_cierre.html)

```
┌─────────────────────────────────────────────────────────────────┐
│ REFACCIONARIA - ACCESO RAPIDO                  Usuario: Admin   │
│                                               [Cerrar Sesión]   │
├─────────────────────────────────────────────────────────────────┤
│
│ Cajas > Cierres de Caja
│
│ Lista de cierres de caja
│
│ ┌────────────────────────────────────────────────────────────┐
│ │ Sucursal: REFACCIONARIA OVIEDO    [➕ Nuevo Cierre]      │  ← NUEVO
│ └────────────────────────────────────────────────────────────┘
│
│ ┌─ Filtros ─────────────────────────────────────────────────────┐
│ │ Caja: [____]  Fechas: [__/__/__] a [__/__/__]  Vendedor: [_] │
│ │                                              [Buscar]         │
│ └───────────────────────────────────────────────────────────────┘
│
│ ┌─ Resultados ───────────────────────────────────────────────────┐
│ │ CAJA   | VENDEDOR             | APERTURA    | CIERRE    [Menu]│
│ ├────────┼──────────────────────┼─────────────┼──────────────────┤
│ │VENTAS01│ REINALDO OVIEDO      │ 05/01/2026  │ 05/01/2026      │
│ │        │ CALIXTRO             │ 09:12       │ 18:20           │
│ └────────┴──────────────────────┴─────────────┴──────────────────┘
│
└─────────────────────────────────────────────────────────────────┘
```

## 📋 Vista 2: Formulario Nuevo Cierre (cierre_caja_nuevo.html) - NUEVO

```
┌─────────────────────────────────────────────────────────────────┐
│ [← Volver]  Nuevo Cierre de Caja          Usuario: REINALDO     │
├─────────────────────────────────────────────────────────────────┤
│
│ ┌─ Datos del Cierre ────────────────────────────────────────────┐
│ │ Caja: [VENTAS01]          Fecha/Hora: 15/01/2026  10:30:45  │
│ │ Vendedor: REINALDO OVIEDO CALIXTRO                            │
│ │ Sucursal: REFACCIONARIA OVIEDO                                │
│ └───────────────────────────────────────────────────────────────┘
│
│ ┌─ Seleccione opciones correspondientes a su cierre ───────────┐
│ │ Formas de Pago:                                               │
│ │ ☑ Efectivo  [        1000.00]  │  ☑ Crédito   [         500] │
│ │ ☐ Cheque    [                ]  │  ☐ Vale      [             │
│ │ ☐ Tarjeta   [                ]  │  ☐ Lealtad   [             │
│ │ ☐ Débito    [                ]  │  ☐ Retiros   [         200] │
│ │ ☐ Depósito  [                ]  │                             │
│ │                                                                │
│ │ Total ingresos (sin retiros):               $1,500.00         │
│ │ Total cierre (ingresos - retiros):          $1,300.00         │
│ │                                                                │
│ │ Solo se consideran los conceptos marcados.                    │
│ │ El total se actualiza automáticamente.                        │
│ │                                                                │
│ │                           [Cancelar]  [Procesar] ←─ GUARDAR   │
│ └───────────────────────────────────────────────────────────────┘
│
└─────────────────────────────────────────────────────────────────┘
```

## 🔄 Flujo de Interacción

```
User en cajas_cierre.html
           │
           └─→ Clic en "➕ Nuevo Cierre"
                      │
                      ↓
           Abre cierre_caja_nuevo.html
                      │
                      ├─→ Auto-carga datos del usuario:
                      │   - Fecha/Hora actual
                      │   - Vendedor (from localStorage)
                      │   - Sucursal (from localStorage)
                      │
                      ├─→ Usuario marca checkboxes y
                      │   ingresa montos
                      │   │
                      │   └─→ JavaScript recalcula:
                      │       - total_ingresos
                      │       - total_cierre
                      │
                      └─→ Clic en "Procesar"
                           │
                           ↓
                POST /api/v1/cajas/cierres
                (JSON con montos)
                           │
                           ↓
                Backend calcula totales
                Guarda en BD cierres_caja
                           │
                           ↓
                Alert: "Cierre creado. Folio: 42"
                           │
                           ↓
                Redirige a cajas_cierre.html
```

## 📊 Estructura de Datos Guardados

```
Tabla: cierres_caja

┌────────┬──────────┬────────────┬─────────┬─────────┬──────────┬────────────┬──────────────┐
│ id     │ caja     │ usuario_id │ efectivo│ cheque  │ credito  │ retiros    │ total_cierre │
├────────┼──────────┼────────────┼─────────┼─────────┼──────────┼────────────┼──────────────┤
│ 1      │ VENTAS01 │ 5          │ 1000.00│ 0.00    │ 500.00   │ 200.00     │ 1300.00      │
│ 2      │ VENTAS02 │ 6          │ 2500.00│ 1000.00 │ 0.00     │ 0.00       │ 3500.00      │
└────────┴──────────┴────────────┴─────────┴─────────┴──────────┴────────────┴──────────────┘

Total de 9 campos de montos:
  - efectivo, cheque, tarjeta, debito, deposito
  - credito, vale, lealtad, retiros
  
Campos calculados automáticamente:
  - total_ingresos = efectivo + cheque + tarjeta + debito + deposito + credito + vale + lealtad
  - total_cierre = total_ingresos - retiros
```

## 🎯 Características

✅ **Interfaz Intuitiva**
   - Diseño similar a la imagen proporcionada
   - Botones claramente identificados
   - Fecha/hora automáticas
   - Datos del usuario precargados

✅ **Cálculos Automáticos**
   - En tiempo real mientras se ingresan montos
   - Recálculo al marcar/desmarcar checkboxes
   - Validación en backend (no se confía en cliente)

✅ **Seguridad**
   - Requiere autenticación (token)
   - Datos del usuario obtenidos del sistema
   - Transacciones garantizadas

✅ **Integración**
   - Parte del menú principal
   - Navega correctamente
   - URLs limpias y semánticas

---

## 🚀 URLs Principales

| Página | URL |
|--------|-----|
| Cierres de Caja | `/static/cajas_cierre.html` |
| Nuevo Cierre | `/static/cierre_caja_nuevo.html` |
| Ruta del menú | `/cajas-cierre` |
| API endpoint | `POST /api/v1/cajas/cierres` |

---

## 💾 Persistencia

- **Tabla BD**: `cierres_caja`
- **Modelo**: `app.models.cierre_caja.CierreCaja`
- **Automático**: La tabla se crea al iniciar la aplicación
- **Backup SQL**: `scripts/create_cierres_caja_table.sql`
